...
//
// Everything here is a native namespace: keyvalue (tasks), sessions
// (isolation), vfs {session} (draft writes), notifications (review), and
// the workspace's LLM alias globals.
...
for (const entry of queue.slice(0, 2)) {
  ...
  // Dotted namespaces are also injected under sanitized aliases.
  const llm = globalThis[providerId] || globalThis[providerId.replace(/[^A-Za-z0-9_]/g, "_")];
