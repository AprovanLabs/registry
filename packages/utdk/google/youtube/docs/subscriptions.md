...
```ts
youtube.youtubeSubscriptionsDelete(input: {
  /** V1 error format. */
  "$.xgafv"?: "1" | "2";
  /** OAuth access token. */
  access_token?: string;
...
<sub>`GET /youtube/v3/subscriptions` · `youtube.subscriptions.list`</sub>

## `youtube.youtubeSubscriptionsInsert`

Inserts a new resource into this collection. — [Provider docs](https://developers.google.com/youtube/)

```ts
youtube.youtubeSubscriptionsInsert(input: {
...
<sub>`POST /youtube/v3/subscriptions` · `youtube.subscriptions.insert`</sub>
